...
@Component({
  selector: 'app-own-meal',
  templateUrl: './own-meal.component.html',
  styleUrls: ['./own-meal.component.css']
})
export class OwnMealComponent implements OnInit {
  @ViewChild('sub_btn')
  myDiv!: ElementRef;
  @ViewChild('f')
  SubmitForm!: NgForm;
  ...
  OnSubmit() {
    if (!this.SubmitForm.valid || this.SubmitForm.value.activity == -1) {
      ...
      if (this.show_us_fields) {
        ...
        this.user_data.hEight = (this.SubmitForm.value.height_feet * 30.48);
        
        // + (this.SubmitForm.value.height_inch * 2.54);
      }
      else {
